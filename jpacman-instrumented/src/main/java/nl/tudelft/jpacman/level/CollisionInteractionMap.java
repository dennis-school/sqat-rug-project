...
import nl.tudelft.jpacman.board.Unit; import static nl.dennis.api.CoverageChecker.hit; public class CollisionInteractionMap implements CollisionMap {
	...
	public <C1 extends Unit, C2 extends Unit> void onCollision(
			...
			CollisionHandler<C1, C2> handler) {hit("nl.tudelft.jpacman.level.CollisionInteractionMap","onCollision"); hit("nl.tudelft.jpacman.level.CollisionInteractionMap","onCollision",1); onCollision(collider, collidee, true, handler); hit("nl.tudelft.jpacman.level.CollisionInteractionMap","onCollision",2);}
	...
	public <C1 extends Unit, C2 extends Unit> void onCollision(
			...
			CollisionHandler<C1, C2> handler) {hit("nl.tudelft.jpacman.level.CollisionInteractionMap","onCollision"); hit("nl.tudelft.jpacman.level.CollisionInteractionMap","onCollision",2); addHandler(collider, collidee, handler); hit("nl.tudelft.jpacman.level.CollisionInteractionMap","onCollision",3); if (symetric) {
			addHandler(collidee, collider, new InverseCollisionHandler<>(
					handler)); hit("nl.tudelft.jpacman.level.CollisionInteractionMap","onCollision",1);
		} hit("nl.tudelft.jpacman.level.CollisionInteractionMap","onCollision",4);}
	...
	private void addHandler(Class<? extends Unit> collider,
			Class<? extends Unit> collidee, CollisionHandler<?, ?> handler) {hit("nl.tudelft.jpacman.level.CollisionInteractionMap","addHandler"); hit("nl.tudelft.jpacman.level.CollisionInteractionMap","addHandler",2); if (!handlers.containsKey(collider)) {
			handlers.put(
					collider,
					new HashMap<>()); hit("nl.tudelft.jpacman.level.CollisionInteractionMap","addHandler",1);
		} hit("nl.tudelft.jpacman.level.CollisionInteractionMap","addHandler",3); Map<Class<? extends Unit>, CollisionHandler<?, ?>> map = handlers
				.get(collider); hit("nl.tudelft.jpacman.level.CollisionInteractionMap","addHandler",4); map.put(collidee, handler); hit("nl.tudelft.jpacman.level.CollisionInteractionMap","addHandler",5);}
	...
	private Class<? extends Unit> getMostSpecificClass(
			Map<Class<? extends Unit>, ?> map, Class<? extends Unit> key) {hit("nl.tudelft.jpacman.level.CollisionInteractionMap","getMostSpecificClass"); hit("nl.tudelft.jpacman.level.CollisionInteractionMap","getMostSpecificClass",3); List<Class<? extends Unit>> collideeInheritance = getInheritance(key); hit("nl.tudelft.jpacman.level.CollisionInteractionMap","getMostSpecificClass",4); for (Class<? extends Unit> pointer : collideeInheritance) {
			if (map.containsKey(pointer)) {
				return pointer;
			} hit("nl.tudelft.jpacman.level.CollisionInteractionMap","getMostSpecificClass",2);
		} hit("nl.tudelft.jpacman.level.CollisionInteractionMap","getMostSpecificClass",5); return null;}
